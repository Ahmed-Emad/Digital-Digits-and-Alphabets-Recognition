    ...
    for eachRow in array:
        ...
        while ((halfColumns - i >= 0) and (halfColumns + i < len(eachRow))):
            ...
            if (eachRow[halfColumns - i] and (not (eachRow[halfColumns + i]))) \
                ...
                dismatchedCounter += 1
            i += 1   
    features.append(matchedCounter / float(dismatchedCounter + matchedCounter))      # [10] Reflected Y
    ...
    while ((halfRows - i >= 0) and (halfRows + i < rows)):        
        for eachPix in range(columns):
            ...
            if (array[halfRows - i][eachPix] and (not (array[halfRows + i][eachPix]))) \
                ...
                dismatchedCounter += 1
        i += 1   
    features.append(matchedCounter / float(dismatchedCounter + matchedCounter))      # [11] Reflected X
...
def readFile(path) :
    fileArray = open(path + 'trainSet2 copy.txt','r').read()
    # print 'fileArray'
    # print fileArray
...
# features = createTrainSet(range(0, 1), range(1, 12))
# print features
# for feature in features:    
#     print str(feature[8]) + ' - ' + str(feature[9])
